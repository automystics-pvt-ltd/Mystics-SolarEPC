    ...
    >
      ...
      <div className="flex items-center flex-wrap gap-3 px-5 py-3 rounded-xl border bg-card">
        ...
        <button
          ...
          onClick={() => setLocation(`/procurement/pos/${g.poId}`)}
        >#{g.poId}</button>
        {g.quotationId && (
          <>
            <div className="h-4 w-px bg-border/60" />
            <span className="text-[12px] text-muted-foreground">Quotation:</span>
            <button
              className="font-mono text-[12px] font-semibold text-primary hover:underline"
              onClick={() => setLocation(`/procurement/quotations/${g.quotationId}`)}
